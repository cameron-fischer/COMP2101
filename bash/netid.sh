...
#This while loop is used to determine whether there is an input in the shell to work with. If there is a parameter greater than 0, a case is run so that the verbose variable will be run as yes, while the interface variable will be run with the first command line argument. Once an argument has been run, it will shift onto the next one until none remain.
while [ $# -gt 0 ]; do
case "$1" in
	-v )
	verbose="yes"
...
#The interface in this script has been adjusted from eno1 to ens33 since that is a default interface that most systems have.
interface="ens33"
